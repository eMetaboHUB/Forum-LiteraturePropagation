...
Dans notre analyse on ne veut pas qu'une mention du composé $c_k$ contribue au prior de ce même composé $k$. Ainsi, on souhaite que $P(I = k|c_k) = 0$
Ainsi, toutes les publis qui allait sur $k$ sont redistribuées dans le reste du réseau. On pose donc :
Pour $P(I = k|c_k) = 0$ et $\forall j \neq i: P(I = j|C_i) = \frac{\pi_{j,i}}{1 - \pi_{i,i}}$ On conserve $\sum_j P(I = j|C_i) = 1$

Ainsi on lorsque le noeud d'arrivé est différent du noeud de départ, on considère la probabilité d'atteindre ce noeud cible, **sachant** que ce n'est pas le noeud de départ. Et on attribue la probabilité d'atteindre le noeud de départ à 0

Ainsi, pour $i = k$:

$p(c_k|I = k) = \frac{P(I=k|c_k)p(c_k)}{\sum_j P(I=k|c_j)p(c_j)} = 0$ car $P(I = k|c_k) = 0$ Le poids du composé $k$ à son propre prior est null !

pour $i \neq k$:

$p(c_i|I = k) = \frac{P(I=k|c_i)p(c_i)}{\sum_j P(I=k|c_j)p(c_j)} = \frac{\frac{ \pi_{k,i} N_i}{ (1 - \pi_{i,i}) N}}{\sum_j \frac{ \pi_{k,j} N_j}{ (1 - \pi_{j,j}) N}} = \frac{\frac{ \pi_{k,i}}{ (1 - \pi_{i,i})} N_i}{\sum_j \frac{ \pi_{k,j}}{ (1 - \pi_{j,j})} N_j}$
...
Sachant que notre PPR restart au voisins de notre noeud cible, cela nous montre donc la taille moyenne d'une marche partant de nos voisins !

Mais il est également possible de mettre le focus sur le noeud cible:

Ainsi, si on place le focus sur notre noeud cible, alors le fait que l'on restart directment sur ces voisins, nous donne en gros une première marche *gratuite* partant de celui-ci !

On a donc : $P(X = k) = 1 \times \alpha^{k - 1} (1 - \alpha)$

Pour un nombre $k$ de marche réaliser avant de restart, la première marche partant de notre noeud cible vers ses voisins est gratuite (proba de 1) puis il nous reste donc $k - 1$ marche à réaliser avec donc une proba $\alpha^{k - 1}$, puis l'échec, on restart avec une proba $(1 - \alpha)$

Dans ce focus donc :  $P(X = k) = \alpha^{k - 1} (1 - \alpha)$

C'est en fait une autre formulation de la loi géométrique où on représente la probabilité que le premier succès arrive au $k$ième essai: ( $P(X = k) = (1 - p)^{k - 1} p$)

Ainsi, si $P(X = k) = \alpha^{k - 1} (1 - \alpha)$, notre moyenne du nombre de marches effectué, autour de notre noeud cible, est de $\frac{1}{(1 - \alpha)}$ Quand $\alpha = 0$, on a donc notre moyenne qui vaut 1 ! En effet seulement $P(k = 1) = 1$ On ne visite que nos voisins !

De la même manière quand $\alpha$ va tendre vers 1, notre moyenne tend vers l'infini.
