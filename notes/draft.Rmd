...
$p(c_i|I = k)$ représentant la probabilité qu'une mention, arrivée sur le composé cible $k$, provienne de $c_i$.
...
- Ainsi mes poids dans ma mixture : $p(c_i|I = k) = \frac{p(c_i, I=k)}{p(I=k)} = \frac{p(c_i, I=k)}{\sum_jp(c_j, I=k)}= \frac{P(I=k|c_i)p(c_i)}{\sum_j P(I=k|c_j)p(c_j)} = \frac{\frac{\pi_{k,i} N_i}{N}}{\sum_j\frac{\pi_{k,j} N_j}{N}} = \frac{\pi_{k,i} N_i}{\sum_j \pi_{k,j} N_j}$
...
En effet, si on voulait déterminer $p(m|c_i, I=k)$ on ferait $p(m|c_i, I=k) = \frac{\pi_{k,i} N_i^m}{\pi_{k,i} N_i} = \frac{N_i^m}{N_i} = p(m|c_i)$
